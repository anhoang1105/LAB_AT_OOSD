...
namespace QuanLyKhachSanLAB3.Services
{
    public class DanhMucService
    {
        ...
        public KetQuaXuLy ThemKhu(string ma, string ten)
        {
            ...
            try
            {
                Db.Execute("INSERT INTO KhuVuc(MaKhuVuc, TenKhuVuc) VALUES(@m, @t)", new SqlParameter("@m", ma), new SqlParameter("@t", ten));
        ...
        {
            if (string.IsNullOrWhiteSpace(ma) || string.IsNullOrWhiteSpace(ten) || string.IsNullOrWhiteSpace(vaiTro)) return KetQuaXuLy.Fail("Dữ liệu trống.");
            try
            {
                Db.Execute("INSERT INTO NhanVien(MaNV, HoTen, VaiTro, SoDienThoai) VALUES(@m, @t, @v, @s)", new SqlParameter("@m", ma), new SqlParameter("@t", ten), new SqlParameter("@v", vaiTro), new SqlParameter("@s", (object)sdt ?? DBNull.Value));
        ...
        public KetQuaXuLy ThemLoaiTN(string ma, string ten)
        {
            ...
            try
            {
                Db.Execute("INSERT INTO LoaiTienNghi(MaLoaiTN, TenLoaiTN) VALUES(@m, @t)", new SqlParameter("@m", ma), new SqlParameter("@t", ten));
        ...
        public KetQuaXuLy ThemDichVu(string ma, string ten, string dvt, decimal gia)
        {
            ...
            try
            {
                Db.Execute("INSERT INTO DichVu(MaDV, TenDV, DonViTinh, DonGia) VALUES(@m, @t, @d, @g)", new SqlParameter("@m", ma), new SqlParameter("@t", ten), new SqlParameter("@d", dvt), new SqlParameter("@g", gia));
        ...
        {
            if (string.IsNullOrWhiteSpace(ma) || string.IsNullOrWhiteSpace(loai) || string.IsNullOrWhiteSpace(muc)) return KetQuaXuLy.Fail("Dữ liệu trống.");
            try
            {
                Db.Execute("INSERT INTO QuyDinhDenBu(MaQuyDinh, MaLoaiTN, MucDoThietHai, MucDenBu) VALUES(@m, @l, @u, @t)", new SqlParameter("@m", ma), new SqlParameter("@l", loai), new SqlParameter("@u", muc), new SqlParameter("@t", tien));

            ...
            using (var tx = cn.BeginTransaction())
            {
                try
                {
                    ...
                    if (string.IsNullOrWhiteSpace(so))
                    {
                        ...
                        // Đã thêm rõ tên cột
                        var h = new SqlCommand("INSERT INTO PhieuSuDungDV(SoPhieuSDDV, SoPhieuDat, SoPhong, NgaySuDung, MaNV) VALUES(@so, @s, @p, @n, @nv)", cn, tx);
                    ...
                    {
                        // Đã bỏ qua cột ThanhTien tự tính trong SQL
                        var i = new SqlCommand("INSERT INTO ChiTietPhieuSuDungDV(SoPhieuSDDV, MaDV, SoLuong, DonGia) VALUES(@so, @d, @sl, @g)", cn, tx);

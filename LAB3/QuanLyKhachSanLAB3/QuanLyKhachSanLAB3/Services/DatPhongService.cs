...
namespace QuanLyKhachSanLAB3.Services
{
    ...
    public class DatPhongService
    {
        ...
        public DataTable LayChiTiet(string so) { return Db.Query("SELECT c.*, p.SoNguoiToiDa, p.DonGiaNgay FROM ChiTietDatPhong c JOIN Phong p ON c.SoPhong = p.SoPhong WHERE c.SoPhieuDat = @s", new SqlParameter("@s", so)); }
        public DataTable LayNguoiLuuTru(string so) { return Db.Query("SELECT * FROM NguoiLuuTru WHERE SoPhieuDat = @s ORDER BY SoPhong, MaNguoiLT", new SqlParameter("@s", so)); }

        public KetQuaXuLy ThemKhach(string ma, string ten, string cmnd, string qt, string sdt)
        {
            if (string.IsNullOrWhiteSpace(ma) || string.IsNullOrWhiteSpace(ten)) return KetQuaXuLy.Fail("Dữ liệu trống.");
            try
            {
                Db.Execute("INSERT INTO KhachHang(MaKhach, HoTen, SoCMND, QuocTich, SoDienThoai) VALUES(@m, @t, @c, @q, @s)", new SqlParameter("@m", ma), new SqlParameter("@t", ten), new SqlParameter("@c", cmnd), new SqlParameter("@q", qt), new SqlParameter("@s", sdt));
        ...
        {
            if (string.IsNullOrWhiteSpace(so) || ds == null || ds.Count == 0) return KetQuaXuLy.Fail("Dữ liệu trống.");
            ...
            using (var tx = cn.BeginTransaction())
            {
                try
                {
                    foreach (var x in ds)
                    {
                        ...
                        if (x.SoNguoi <= 0 || x.SoNguoi > Convert.ToInt32(o)) return KetQuaXuLy.Fail("Số người phòng " + x.SoPhong + " vượt sức chứa.");
                        if (PhongTrungLich(cn, tx, x.SoPhong, nhan, tra)) return KetQuaXuLy.Fail("Phòng " + x.SoPhong + " trùng lịch.");
                    }
        ...
        {
            if (string.IsNullOrWhiteSpace(so) || string.IsNullOrWhiteSpace(ten)) return KetQuaXuLy.Fail("Dữ liệu trống.");
            try
            {
                ...
                if (dem >= max) return KetQuaXuLy.Fail("Đã đủ số người đăng ký.");
        ...
        public KetQuaXuLy NhanPhong(string so, DateTime thucTe)
        {
            ...
            using (var tx = cn.BeginTransaction())
            {
                try
                {
                    ...
                    c.Parameters.AddWithValue("@s", so);
                    if (c.ExecuteNonQuery() == 0) return KetQuaXuLy.Fail("Phiếu không thể nhận phòng.");
